...
9. **The 34-hour restart is implemented for the pre-flight case only.** `PlanningEngine` schedules a restart when the cycle arrives already exhausted at trip start (BR-8/BR-10, AC-11, EC-4/EC-44) — placeable because the truck has not moved, so it needs neither route-geometry interpolation nor mid-leg splitting. Two further cases remain open and belong with timeline generation, not with the cycle rules:

   - **Mid-trip restart at a leg boundary** (EC-20, EC-42) needs the engine's leg loop to resume after scheduling a remedy rather than halt, and needs nearest-binding-constraint selection (see below) so a restart is not scheduled where a 30-minute break was due first.
   - **BR-36's "restart at the last legal point"** needs a leg to be splittable at an arbitrary mileage. `RouteLegInput` carries leg endpoints but no geometry, so a mid-leg position cannot yet be resolved to a place name. This is a shared prerequisite of the break, reset and fuel remedies too, not a cycle-specific gap.

   Related known limitation: `PlanningEngine` returns on the **first** evaluator to block, in priority order. The first rule to block is not necessarily the one that binds soonest — a cycle block with 2 h of cycle remaining will currently preempt a duty-window block with 1 h remaining, which would schedule a 34-hour restart where a 30-minute break was correct. This is harmless while the only remedy scheduled is the pre-flight one (no clock but the cycle has advanced when it runs), and must be fixed before any mid-trip remedy is acted on. The fix demotes `priority()` to a tie-breaker among simultaneously-binding rules.
