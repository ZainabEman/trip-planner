...
| **RuleResult** | The DTO a rule check (or the evaluator as a whole) returns: whether driving may continue for the proposed increment, and if not, which mandatory action is required next (30-min break / 10-hour reset / 34-hour restart / fuel stop), plus the human-readable reason and the rule ID that triggered it (feeds FR-4.2's mandatory `reason` field and BR-33/US-22 traceability). | Never itself performs a side effect — purely a description of "what must happen next and why." The `required_action` field is what keeps a rule's *remedy* out of the rule's own evaluator: **there is deliberately no `34HourRestartEvaluator`** (nor a break/reset/fuel evaluator). A restart never forbids driving, so it cannot be an allowed/blocked verdict without restating `CycleLimitEvaluator`'s 70-hour threshold — one trigger, one remedy, so `ComplianceValidator` has a single threshold to check rather than two that could drift. |
...
| **BR-9** — Rolling 70-hour window | A new day is added to the window | The oldest day's on-duty hours drop out of the rolling total | No standalone event. **Not implemented — a supported simplification; see Assumption 8 below.** The cycle clock only ever grows within a trip |
...
2. **Duty-period granularity for pre-/post-trip inspections. — RESOLVED, confirmed for implementation.** BR-21/BR-22 say inspections bracket "each duty period," but the PRD does not define precisely where one duty period ends and the next begins for a multi-day trip. This design assumed a duty period is bounded by any qualifying 10-hour-or-longer off-duty block (i.e., every 10-hour reset or 34-hour restart both closes one duty period and opens the next, each with its own 15-minute inspection pair). An alternative reading — inspections occur only once at the absolute start and end of the whole trip — would be simpler but seems less faithful to "every real log" (A-16's stated rationale).

   **This reading is now confirmed and implemented** for the one case the engine currently schedules: a 34-hour cycle restart begins a new duty period and therefore emits its own 15-minute pre-trip inspection immediately after the restart completes, with the 14-hour window opening at the *start* of that inspection per BR-24. See `PlanningEngine._apply_preflight_restart`. The same rule will apply to the 10-hour reset when that remedy is scheduled.
...
8. **BR-9's rolling 8-day drop-out is not implemented — a supported v1 simplification.** BR-9 says the oldest day's on-duty hours drop out of the rolling 70-hour total as each new day is added. This **cannot** be implemented from the input the engine is given: the driver's cycle history arrives as the single scalar `Trip.cycle_hours_used` (FR-1.5, range 0–70), which records *how many* hours were used but not *when*. A per-day distribution is not recoverable from one total, so there is no identifiable day whose hours could be dropped.

   The consequence is that the engine's cycle clock only ever grows within a trip, never decays. That is the **conservative** direction: the engine may schedule a 34-hour restart marginally earlier than a driver with a known-favourable 8-day history strictly needs, but it will never permit driving that the true rolling window would forbid. Since BR-37/NFR-2.4 require the engine never emit a non-compliant plan, erring toward an unnecessary restart is the correct failure mode — the opposite error would be a compliance violation.

   Lifting this is an **input-model change, not an engine change**: it requires accepting a per-day on-duty history (a new `Trip` field and a new form input) in place of the scalar. `CycleLimitEvaluator` would then read that history instead of a running total; its threshold logic is unaffected. Recorded in `evaluators/cycle_limit.py` as well.

9. **All four remedies are scheduled, mid-trip and pre-flight.** `PlanningEngine` inserts the 30-minute break (BR-4), the 10-hour reset (BR-7), the fuel stop (BR-19/BR-20) and the 34-hour restart (BR-10) wherever a rule requires one, then resumes from the point it stopped. The three items previously open here are resolved:

   - **Mid-trip restart at a leg boundary** (EC-20, EC-42): the planning loop resumes after scheduling a remedy instead of halting.
   - **BR-36's "restart at the last legal point"**: a leg is split at an arbitrary mileage. The split point's *timing* is exact — it is where the duty clocks run out — while its *position* is interpolated linearly between the leg's endpoints, because `RouteLegInput` still carries no geometry. A mid-leg stop is therefore named generically ("En route to X, N mi into leg K") and pinned approximately. Carrying the encoded polyline into the engine's input would make the pin exact; it is an input-model change, not an engine one.
   - **Nearest-binding-constraint selection**: `priority()` is now only a tie-breaker. Every rule is evaluated on each proposed increment, each blocking rule's remaining budget is converted to hours of driving, and the smallest wins — so a cycle block with 2 h remaining no longer preempts a duty-window block with 1 h remaining, and a 34-hour restart is not scheduled where a 30-minute break was due.

   Remedies remain strictly separate from the rules that trigger them: an evaluator names a `RequiredAction` and nothing else, and `remedies.py` holds a table mapping each action to exactly one remedy — its duration, duty status, event type, and which clocks it clears. No evaluator knows how long a break is; `PlanningEngine` does not know what 70 hours means. This is the R-1 mitigation applied to remedies as well as to thresholds.

   Two guards make the loop safe. A **no-progress detector** stops the plan when the same remedy is requested twice with no driving in between — remedies are idempotent on the clocks they clear, so a repeat cannot help — and a **hard iteration ceiling** (`MAX_PLANNING_ITERATIONS`) sits behind it for any future rule whose remedy neither helps nor repeats. A trip now fails only when the binding rule names no remedy at all, which is why almost every `422` from the plan endpoint is a routing or geocoding failure rather than an HOS one.
